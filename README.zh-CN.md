...
注: 在进程的 CPU 亲和性（affinity）更改后，AffinityServiceRust 会自动重设该进程内各线程的理想处理器 (ideal processor) 分配，以避免 Windows 内核在亲和性更新后将过多线程的理想处理器截断到相同的物理 CPU 核心的问题。
  - 重置逻辑会收集线程的总 CPU 时间与线程周期计数，按总 CPU 时间（降序）为主、周期计数为辅排序，然后在目标亲和性 CPU 上以轮询（round-robin）方式分配理想处理器，并加入小幅随机偏移以避免压力分配不均。
