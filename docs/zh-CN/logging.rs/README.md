...
- **失败映射维护** — [`purge_fail_map`](purge_fail_map.md) 移除已不再运行的进程的陈旧条目。
...
| [PID_MAP_FAIL_ENTRY_SET](PID_MAP_FAIL_ENTRY_SET.md) | 按 PID 的去重错误条目映射，键为 (pid, tid, name, operation, error_code)。 |
...
| 名称 | 说明 |
| --- | --- |
| [ApplyFailEntry](ApplyFailEntry.md) | 错误去重的复合键，组合 pid、tid、进程名称、操作和错误代码。 |
...
| [purge_fail_map](purge_fail_map.md) | 移除已不再运行的进程的陈旧错误条目。 |
...
4. 每次循环调用 [`purge_fail_map`](purge_fail_map.md)，移除已不再存活的 PID 的条目，防止内存无限增长。
