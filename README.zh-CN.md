...
| **理想处理器（首核）分配** | 基于滞后（hysteresis）算法的理想处理器分配，使用与 Prime 线程调度相同的算法和常量（`MIN_ACTIVE_STREAK`、`ENTRY_THRESHOLD`、`KEEP_THRESHOLD`） |
...
**算法：**
- 每次迭代的周期数据由共享的 `prefetch_all_thread_cycles` 预取通道提供（每线程一次 `QueryThreadCycleTime` 调用，数量上限为逻辑 CPU 数）
- 应用 `MIN_ACTIVE_STREAK`、`ENTRY_THRESHOLD`、`KEEP_THRESHOLD` 常量，与 Prime 线程调度完全相同：
...
注：在进程的 CPU 亲和性（affinity）更改后，AffinityServiceRust 会自动重设该进程内各线程的理想处理器分配，以避免 Windows 内核在亲和性更新后将过多线程截断到相同物理 CPU 核心的问题。重置逻辑按总 CPU 时间（降序）排序，并以轮询方式加随机偏移分配理想处理器。
...
4. **共享周期预取**（每进程，prime 与 ideal 调度前）
   ...
   - 仅对前 N 个线程打开句柄并调用 `QueryThreadCycleTime`；结果缓存于 `ThreadStats::cached_cycles`
5. **理想处理器分配**（每进程，每间隔）
