...
- **CPU 亲和性**：将进程硬性绑定到特定的逻辑处理器（旧版 ≤64 核系统）—— 参见 [`apply_affinity()`](docs/apply.md#apply_affinity)
- **CPU 集**：跨所有处理器组的软性 CPU 偏好（现代 >64 核系统）—— 参见 [`apply_process_default_cpuset()`](docs/apply.md#apply_process_default_cpuset)
...
- **I/O 优先级控制**：控制磁盘 I/O 调度优先级 —— 参见 [`apply_io_priority()`](docs/apply.md#apply_io_priority)
- **内存优先级控制**：调整进程工作集的内存页面优先级 —— 参见 [`apply_memory_priority()`](docs/apply.md#apply_memory_priority)
- **热重载**：自动检测并应用配置文件更改
...
| 主题 | 文档 |
|------|------|
| **架构** | [docs/main.md](docs/main.md) - 主循环和入口点 |
| **配置** | [docs/config.md](docs/config.md) - 配置解析和 CPU 规格 |
| **应用逻辑** | [docs/apply.md](docs/apply.md) - 如何将设置应用到进程 |
| **调度器** | [docs/scheduler.md](docs/scheduler.md) - Prime 线程调度器实现 |
| **CLI 选项** | [docs/cli.md](docs/cli.md) - 命令行参数 |
| **优先级级别** | [docs/priority.md](docs/priority.md) - 优先级枚举定义 |
| **Windows API** | [docs/winapi.md](docs/winapi.md) - Windows API 包装器 |
| **日志** | [docs/logging.md](docs/logging.md) - 错误跟踪和日志 |
...
| **CPU 亲和性** | 旧版基于掩码的亲和性（≤64 核，[`SetProcessAffinityMask`](docs/apply.md#apply_affinity)） |
| **CPU 集** | 通过 Windows CPU Sets 软性偏好核心（支持 >64 核，[`SetProcessDefaultCpuSets`](docs/apply.md#apply_process_default_cpuset)） |
| **Prime 线程调度** | 动态分配 CPU 密集型线程到性能核心 |
| **理想处理器（首核）分配** | 基于滞后（hysteresis）算法的理想处理器分配，使用与 Prime 线程调度相同的算法和常量（[`MIN_ACTIVE_STREAK`](docs/config.md#configconstants)、[`ENTRY_THRESHOLD`](docs/config.md#configconstants)、[`KEEP_THRESHOLD`](docs/config.md#configconstants)） |
...
> **关于 >64 核系统的说明：** CPU 亲和性（[`SetProcessAffinityMask`](docs/apply.md#apply_affinity)）只能在单个处理器组内工作（≤64 核）。对于 >64 核系统，请使用 CPU 集，它可以跨所有处理器组工作，但为软性偏好。
...
**算法：**
- 在可配置的时间间隔通过 [`prefetch_all_thread_cycles()`](docs/apply.md#prefetch_all_thread_cycles) 监控线程 CPU 周期消耗
- 应用滞后机制防止频繁切换：
...
详情请参见 [`apply_prime_threads()`](docs/apply.md#apply_prime_threads) 和 [scheduler 模块](docs/scheduler.md)。
...
**算法：**
- 每次迭代的周期数据由共享的 [`prefetch_all_thread_cycles()`](docs/apply.md#prefetch_all_thread_cycles) 预取通道提供（每线程一次 `QueryThreadCycleTime` 调用，数量上限为逻辑 CPU 数）
- 应用 [`MIN_ACTIVE_STREAK`](docs/config.md#configconstants)、[`ENTRY_THRESHOLD`](docs/config.md#configconstants)、[`KEEP_THRESHOLD`](docs/config.md#configconstants) 常量，与 Prime 线程调度完全相同：
...
详情请参见 [`apply_ideal_processors()`](docs/apply.md#apply_ideal_processors)。
...
详情请参见 [`reset_thread_ideal_processors()`](docs/apply.md#reset_thread_ideal_processors)。
...
完整 CLI 文档请参见 [cli.md](docs/cli.md)。
...
解析后的表示请参见 [`ProcessConfig`](docs/config.md#processconfig) 结构体。
...
解析实现请参见 [`parse_cpu_spec()`](docs/config.md#parse_cpu_spec)。
...
所有优先级级别定义请参见 [priority.md](docs/priority.md)。
...
结构体定义请参见 [`ConfigConstants`](docs/config.md#configconstants)。
...
实现请参见 [`sort_and_group_config()`](docs/config.md#sort_and_group_config)。
...
1. **初始化**
   - 解析命令行参数
   - 通过 [`read_config()`](docs/config.md#read_config) 加载并验证配置文件
   - 请求管理员提权（除非 `-noUAC`）
   - 启用 [`SeDebugPrivilege`](docs/winapi.md#enable_debug_privilege) 和 [`SeIncreaseBasePriorityPrivilege`](docs/winapi.md#enable_inc_base_priority_privilege)
   - 设置计时器分辨率（如果指定）
   - 终止从启动器继承的任何子进程（如计划任务运行器附加的 `conhost.exe`），在进入主循环前执行清理

2. **主循环**（每个间隔，默认 5000ms）
   - 通过 [`NtQuerySystemInformation`](docs/process.md#processsnapshot) 获取所有运行进程的快照
   - 对于每个匹配配置规则的进程：
     - 应用进程优先级
     - 应用 CPU 亲和性（通过 [`SetProcessAffinityMask`](docs/apply.md#apply_affinity) 硬性限制）
     - 应用 CPU 集（通过 [`SetProcessDefaultCpuSets`](docs/apply.md#apply_process_default_cpuset) 软性偏好）
     - 应用 prime 线程调度（动态线程到核心分配）
     - 应用 I/O 优先级（通过 [`NtSetInformationProcess`](docs/apply.md#apply_io_priority)）
     - 应用内存优先级（通过 [`SetProcessInformation`](docs/apply.md#apply_memory_priority)）
   - 记录所有更改
   - 清理已死进程/线程句柄
   - 休眠直到下一个间隔
...
3. **Prime 线程调度**（每个进程，每个间隔）
   ...
   - 提升超过入场阈值且连击充足的线程
   - 降级低于保持阈值的线程
   - 通过 [`SetThreadSelectedCpuSets`](docs/scheduler.md#primethreadscheduler) 应用 CPU 集
   - 可选提升线程优先级（自动或显式）
...
4. **共享周期预取**（每进程，prime 与 ideal 调度前）
   ...
   - 仅对前 N 个线程打开句柄并调用 `QueryThreadCycleTime`；结果缓存于 [`ThreadStats::cached_cycles`](docs/scheduler.md#threadstats)
...
| 文件 | 说明 | 文档 |
|------|------|------|
| `src/main.rs` | 主循环、配置应用逻辑 | [main.md](docs/main.md) |
| `src/config.rs` | 配置文件解析、CPU 规格解析、验证 | [config.md](docs/config.md) |
| `src/cli.rs` | 命令行参数解析 | [cli.md](docs/cli.md) |
| `src/priority.rs` | 优先级枚举（进程、线程、I/O、内存） | [priority.md](docs/priority.md) |
| `src/logging.rs` | 日志基础设施（控制台和文件） | [logging.md](docs/logging.md) |
| `src/process.rs` | 进程枚举和快照管理 | [process.md](docs/process.md) |
| `src/scheduler.rs` | Prime 线程调度器实现 | [scheduler.md](docs/scheduler.md) |
| `src/apply.rs` | 将配置应用到进程 | [apply.md](docs/apply.md) |
| `src/winapi.rs` | Windows API 包装器、模块解析、权限处理 | [winapi.md](docs/winapi.md) |
| `config.ini` | 默认配置文件 | - |
...
8. **`[OPEN][ACCESS_DENIED]` 按 PID 去重**：当 [`apply_config()`](docs/main.md#apply_config) 因 `ACCESS_DENIED` 无法打开某进程时，该错误仅对每个唯一的 `(pid, 进程名)` 组合写入一次 `.find.log`。每次获取快照后，去重映射表会与当前快照对账：PID 已退出或被其他可执行文件复用的条目将被清除，因此若同一进程名在新 PID 下再次出现，错误将重新触发一次。同名可执行文件的多个并发实例（如具有不同 PID 的多个 `svchost.exe`）被独立跟踪——某个实例被拒绝访问，不会压制其他同名但不同 PID 实例的错误输出。

错误去重实现请参见 [`is_new_error()`](docs/logging.md#is_new_error)。
