...
| **CPU 亲和性** | 限制进程到指定核心（硬性限制，子进程继承） |
| **CPU 集** | 通过 Windows CPU 集分配首选核心（软性偏好） |
| **Prime Core 调度** | 将最活跃的线程固定到指定高性能核心 |
| **I/O 优先级** | 控制 I/O 优先级（Very Low → High，High 需要管理员） |
| **内存优先级** | 控制内存页面优先级（Very Low → Normal） |
| **多 CPU 组** | 支持 >64 逻辑处理器的系统 |
| **计时器分辨率** | 调整 Windows 计时器分辨率 |

### Prime Core 调度

针对多线程应用（如游戏），此功能识别 CPU 密集型线程并将其固定到指定核心：
...
适用于游戏中主线程/渲染线程需要运行在 P 核，同时避开核心 0/1（硬件中断处理器）的场景。
