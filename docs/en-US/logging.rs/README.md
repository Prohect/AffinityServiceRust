...
- **Fail map maintenance** — [`purge_fail_map`](purge_fail_map.md) removes stale entries for processes that are no longer running.
...
| [PID_MAP_FAIL_ENTRY_SET](PID_MAP_FAIL_ENTRY_SET.md) | Per-PID map of deduplicated error entries, keyed by (pid, tid, name, operation, error_code). |
...
| Name | Description |
| --- | --- |
| [ApplyFailEntry](ApplyFailEntry.md) | Composite key for error deduplication, combining pid, tid, process name, operation, and error code. |
...
| [purge_fail_map](purge_fail_map.md) | Removes stale error entries for processes that are no longer running. |
...
1. Each error is represented as an [`ApplyFailEntry`](ApplyFailEntry.md) keyed by `(pid, tid, process_name, operation, error_code)`.
...
4. [`purge_fail_map`](purge_fail_map.md) is called each loop to remove entries for PIDs that are no longer alive, preventing unbounded memory growth.
...
| **Called by** | All modules via `log!` macro; [`apply_config`](../main.rs/apply_config.md) indirectly via apply functions |
