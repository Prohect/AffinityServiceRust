...
- **[进程优先级管理](#进程优先级)**：设置进程优先级类（从空闲到实时）
- **[CPU 亲和性](#cpu-亲和性)**：将进程硬性绑定到特定的逻辑处理器（旧版 ≤64 核系统）
- **[CPU 集](#cpu-集)**：跨所有处理器组的软性 CPU 偏好（现代 >64 核系统）
- **[Prime 线程调度](#prime-线程调度)**：动态识别并分配 CPU 密集型线程到指定的"prime"核心
- **[理想处理器分配](#理想处理器分配)**：为最繁忙的 N 个线程静态分配线程到 CPU
- **[I/O 优先级控制](#io-优先级)**：控制磁盘 I/O 调度优先级
- **[内存优先级控制](#内存优先级)**：调整进程工作集的内存页面优先级
- **[热重载](#热重载)**：自动检测并应用配置文件更改
- **[规则等级](#规则等级)**：控制每个进程规则的应用频率
