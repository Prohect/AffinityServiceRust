...
`prime_cpus` 字段支持多段式 CPU 分配、每前缀线程优先级控制以及线程跟踪：

#### 多段式语法
```
[??x|?x]*alias1@prefix1[!priority];prefix2;*alias2@prefix3[!priority];...
```

- `?x*prime_cpus` - 跟踪并记录退出时 CPU 周期消耗前 `x` 名的线程，同时应用规则
- `??x*prime_cpus` - 仅监控：跟踪并记录退出时前 `x` 名的线程，但不应用 prime 线程规则
...
- `?20*pN01` - 应用规则使用 *pN01 CPU，并在退出时记录前 20 个线程
- `??20*pN01` - 仅监控：在退出时记录前 20 个线程，不应用 CPU 集
