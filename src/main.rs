...
///
/// This function scans `.find.log` files in the specified logs directory (default: "logs") for process names,
/// filters out processes already configured in the config file or blacklisted, and uses the Everything search
/// tool (`es.exe`) to find file paths for the remaining processes. Results are saved to the output file
/// (default: "new_processes_results.txt").
///
/// # Requirements
...
///
/// # Arguments
/// * `config_path` - Path to the config file to check for existing processes
/// * `blacklist_path` - Optional path to blacklist file (default: none)
/// * `logs_path` - Optional directory containing .find.log files (default: "logs")
/// * `output_file` - Optional output file path (default: "new_processes_results.txt")
///
/// # Process
/// 1. Extracts process names from log lines containing "find " or "[OPEN][INVALID_PARAMETER]"
/// 2. Filters out processes already in config or blacklist
/// 3. Searches for each new process using `es -r ^{proc}$` (exact match)
/// 4. Outputs results with process name and found file paths
